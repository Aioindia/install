...
main ()
{
  ...
  apt_source_path="/etc/apt/sources.list.d/pufferpanel_pufferpanel.list"
  apt_keyrings_dir="/etc/apt/keyrings"
  if [ ! -d "$apt_keyrings_dir" ]; then
    install -d -m 0755 "$apt_keyrings_dir"
  fi
  gpg_keyring_path="$apt_keyrings_dir/pufferpanel_pufferpanel-archive-keyring.gpg"
    gpg_key_path_old="/etc/apt/trusted.gpg.d/pufferpanel_pufferpanel.gpg"
